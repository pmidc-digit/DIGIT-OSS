...
const WSWaterConnectionDetails = ({
  ...
  const [proposedTaps, setProposedTaps] = useState(
    formData?.waterConectionDetails?.proposedTaps || ""
  );
  const [proposedPipeSize, setProposedPipeSize] = useState(
    formData?.waterConectionDetails?.proposedPipeSize || ""
  );
  ...
  const handleSubmit = () => {
    if (
      (!(
        formData?.WaterConnectionResult &&
        formData?.WaterConnectionResult?.WaterConnection?.[0]?.id
      ) &&
        formData?.serviceName?.code === "WATER") ||
      ...
      if (formData?.isModifyConnection) {
        ...
        payload = {
          WaterConnection: {
            ...
            connectionHolders: formData?.ConnectionHolderDetails?.isOwnerSame
              ? null
              : [
                  {
                    correspondenceAddress:
                      formData?.ConnectionHolderDetails?.address,
                    fatherOrHusbandName:
                      formData?.ConnectionHolderDetails?.guardian,
                    gender: formData?.ConnectionHolderDetails?.gender?.code,
                    mobileNumber:
                      formData?.ConnectionHolderDetails?.mobileNumber,
                    name: formData?.ConnectionHolderDetails?.name,
                    ownerType:
                      formData?.ConnectionHolderDetails?.specialCategoryType
                        ?.code || "NONE",
                    relationship:
                      formData?.ConnectionHolderDetails?.relationship?.code,
            ...
            noOfToilets: null,
            propertyId:
              formData?.cptId?.id || formData?.cpt?.details?.propertyId,
            additionalDetails: {
  ...
  console.log(config, "ccccccccccc");
  return (
    <div>
